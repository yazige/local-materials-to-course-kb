...
7. Before conversion, if the user provides a folder path or multiple Word document paths, follow the Folder And Multi-Word Intake Gate. Otherwise convert the source document to Markdown or text. For video/audio materials, first look for exported documents and process them under the Media Export Package Gate instead of treating raw media as the default input. For scanned PDFs or images, use OCR and mark uncertain recognition. For PowerPoint files, extract visible slide text, tables/charts, images, and speaker notes from the presenter view; keep speaker notes tied to slide numbers and mark notes extraction failures as `备注待人工确认`.
...
Do not create a separate video/audio skill by default. Treat video/audio training materials as document packages after they are exported by tools such as 同义听悟 or other transcription systems.
...
- If new materials arrived while a previous batch was unfinished, the new materials were registered separately and not silently mixed into the previous official writes.
...
- If `TBD` still has remaining materials after one batch, they were left in `TBD` for the next automation run.
...
- Case-list tables were filled when cases or images were added; do not leave useful entries only in narrative sections.
